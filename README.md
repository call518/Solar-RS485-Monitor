...
`DASHBOARD_AUTH_ENABLED` enables the built-in dashboard login. `DASHBOARD_AUTH_USERS` stores comma-separated `username:password_hash` entries generated by `solar-rs485-monitor-dashboard --hash-password`. `DASHBOARD_AUTH_COOKIE_SECRET` signs the browser login cookie, and `DASHBOARD_AUTH_COOKIE_MAX_AGE_SECONDS` controls how long non-admin logins survive refreshes and reconnects. The `admin` user receives a long-lived cookie and stays logged in until logout unless the cookie is removed or the cookie secret changes.
