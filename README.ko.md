...
선택적 로깅 sink는 `src/solar_rs485_monitor/sinks/` 아래의 별도 모듈로 구현합니다. 이렇게 해서 인버터 수집 로직과 Google Sheets, ThingSpeak, MariaDB, 향후 OpenSearch 또는 Elasticsearch 같은 외부 로깅 연동을 분리합니다.
...
외부 로깅 실패는 서로 분리되어 처리됩니다. Google Sheets, ThingSpeak, MariaDB가 credential 누락, 인증 실패, 네트워크 오류, rate limit, 데이터베이스 연결 문제 등으로 실패하면 해당 sink의 오류 JSON만 출력하고 나머지 작업은 계속 진행합니다. 한 sink의 실패가 인버터 수집을 중단하거나 다른 활성 sink 실행을 막지 않습니다.
...
`--thingspeak`를 사용하려면 `.env`에 ThingSpeak Write API Key를 설정합니다.
...
`--mariadb`를 사용하려면 `.env`에 다음 값을 설정합니다.
...
- `MARIADB_PASSWORD is not set`: `--mariadb` 실행 전에 `.env`에 MariaDB 비밀번호를 설정합니다.
