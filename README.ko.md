...
선택적 로깅 sink는 `src/solar_rs485_monitor/sinks/` 아래의 별도 모듈로 구현합니다. 이렇게 해서 인버터 수집 로직과 Google Sheets, ThingSpeak, MariaDB, OpenSearch 또는 Elasticsearch 같은 외부 로깅 연동을 분리합니다.
...
`--all-sinks`에서 OpenSearch는 `OPENSEARCH_URL`이 설정된 경우에만 활성화됩니다. OpenSearch 설정 누락을 오류로 확인하고 싶다면 `--opensearch`를 명시적으로 사용합니다.

외부 로깅 실패는 서로 분리되어 처리됩니다. Google Sheets, ThingSpeak, MariaDB, OpenSearch가 credential 누락, 인증 실패, 네트워크 오류, rate limit, 데이터베이스 연결 문제 등으로 실패하면 해당 sink의 오류 JSON만 출력하고 나머지 작업은 계속 진행합니다. 한 sink의 실패가 인버터 수집을 중단하거나 다른 활성 sink 실행을 막지 않습니다.
...
서비스에서 일부 sink만 사용하려면 `--all-sinks` 대신 `--mariadb` 또는 `--thingspeak --mariadb --opensearch` 같은 명시적 옵션으로 바꿉니다.
...
CREATE TABLE IF NOT EXISTS inverter_log (
    ...
    timestamp            DATETIME(6) NOT NULL COMMENT 'TIMEZONE 설정 기준 측정 시각',
    inverter_name        VARCHAR(100) NOT NULL,
    inverter_id          TINYINT UNSIGNED NOT NULL,
    input_dc_voltage_v   SMALLINT UNSIGNED COMMENT 'DC 입력 전압 (V)',
    input_dc_current_a   FLOAT(5,2) COMMENT 'DC 입력 전류 (A)',
    input_dc_power_w     INT UNSIGNED COMMENT 'DC 입력 전력 (W)',
    output_ac_voltage_v  SMALLINT UNSIGNED COMMENT 'AC 출력 전압 (V)',
    output_ac_current_a  FLOAT(5,2) COMMENT 'AC 출력 전류 (A)',
    output_ac_power_w    INT UNSIGNED COMMENT 'AC 출력 전력 (W)',
    output_ac_power_factor_pct FLOAT(5,2) COMMENT 'AC 출력 역률 (%)',
    output_ac_frequency_hz     FLOAT(5,2) COMMENT 'AC 출력 주파수 (Hz)',
    total_generation_kwh FLOAT(10,3) COMMENT '총 발전량 (kWh)',
    fault_code           SMALLINT UNSIGNED DEFAULT 0 COMMENT '고장 코드',
    fault                TINYINT(1) DEFAULT 0 COMMENT '고장 여부 (0/1)',
    created_at           TIMESTAMP DEFAULT CURRENT_TIMESTAMP COMMENT 'DB 입력 시각',
    INDEX idx_timestamp (timestamp),
    INDEX idx_inverter_id (inverter_id),
    INDEX idx_fault (fault)
) ENGINE=InnoDB
  DEFAULT CHARSET=utf8mb4
  COMMENT='solar-rs485-monitor 인버터 로그';
...
이미 예전 `pv_*`, `grid_*`, `current_output_w`, `power_factor_pct`, `frequency_hz` 컬럼명으로 테이블을 생성했다면 한 번만 마이그레이션합니다.

```sql
ALTER TABLE inverter_log
    CHANGE pv_voltage_v input_dc_voltage_v SMALLINT UNSIGNED COMMENT 'DC 입력 전압 (V)',
    CHANGE pv_current_a input_dc_current_a FLOAT(5,2) COMMENT 'DC 입력 전류 (A)',
    CHANGE pv_power_w input_dc_power_w INT UNSIGNED COMMENT 'DC 입력 전력 (W)',
    CHANGE grid_voltage_v output_ac_voltage_v SMALLINT UNSIGNED COMMENT 'AC 출력 전압 (V)',
    CHANGE grid_current_a output_ac_current_a FLOAT(5,2) COMMENT 'AC 출력 전류 (A)',
    CHANGE current_output_w output_ac_power_w INT UNSIGNED COMMENT 'AC 출력 전력 (W)',
    CHANGE power_factor_pct output_ac_power_factor_pct FLOAT(5,2) COMMENT 'AC 출력 역률 (%)',
    CHANGE frequency_hz output_ac_frequency_hz FLOAT(5,2) COMMENT 'AC 출력 주파수 (Hz)';
```
...
| --- | ---: | ---: | --- | --- |
| `@timestamp` | N/A | N/A | timezone offset 포함 ISO 8601 | `TIMEZONE`을 사용해 수집기가 생성한 수집 시각 |
| `inverter_name` | N/A | N/A | text | `INVERTER_NAME` 값 |
| `inverter_id` | frame byte 1 | 1 | numeric ID | 장치가 반환한 인버터 ID |
| `input_dc_voltage_v` | data 0-1 | 1 | V | PV 쪽 DC 입력 전압 |
| `input_dc_current_a` | data 2-3 | 1 | A | PV 쪽 DC 입력 전류 |
| `input_dc_power_w` | data 4-5 | 1 | W | PV 쪽 DC 입력 전력 |
| `output_ac_voltage_v` | data 6-7 | 1 | V | 계통 쪽 AC 출력 전압 |
| `output_ac_current_a` | data 8-9 | 1 | A | 계통 쪽 AC 출력 전류 |
| `output_ac_power_w` | data 10-11 | 1 | W | 현재 AC 출력 전력 |
| `output_ac_power_factor_pct` | data 12-13 | 0.1 | % | AC 출력 역률 백분율 |
| `output_ac_frequency_hz` | data 14-15 | 0.1 | Hz | AC 출력 주파수 |
...
- `Invalid TIMEZONE`: `TIMEZONE`을 `Asia/Seoul` 같은 올바른 IANA timezone 이름으로 설정합니다.
