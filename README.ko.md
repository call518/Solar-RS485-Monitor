...
| 분류 | 메트릭 | 의미 |
| --- | --- | --- |
| 메타데이터 | `@timestamp` | 설정된 `TIMEZONE` 기준 수집 시각 |
...
| 상태 | `fault` | `fault_code`에서 파생한 고장 여부 |
| 디버그 | `raw_frame_hex` | 문제 확인용 원시 응답 프레임 |

같은 파싱 결과는 JSON으로 출력할 수 있고, 선택적으로 Google Sheets, ThingSpeak, MariaDB, OpenSearch 또는 Elasticsearch에 기록할 수 있습니다.
