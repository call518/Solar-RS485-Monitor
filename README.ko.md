...
선택적 로깅 sink는 `src/solar_rs485_monitor/sinks/` 아래의 별도 모듈로 구현합니다. 이렇게 해서 인버터 수집 로직과 Google Sheets, ThingSpeak, 향후 OpenSearch 또는 Elasticsearch 같은 외부 로깅 연동을 분리합니다.
...
외부 로깅 실패는 서로 분리되어 처리됩니다. Google Sheets 또는 ThingSpeak가 API key 누락, 인증 실패, 네트워크 오류, rate limit 등으로 실패하면 해당 sink의 오류 JSON만 출력하고 나머지 작업은 계속 진행합니다. 한 sink의 실패가 인버터 수집을 중단하거나 다른 활성 sink 실행을 막지 않습니다.
