...
본 프로젝트는 한국에너지공단이 주관하는 REMS(신재생에너지 통합모니터링 시스템) 표준 프로토콜 지원을 기본 기준으로 설계되었습니다. 현재는 REMS 중심으로 검증·지원하고 있으며, 범용 Modbus RTU/TCP 병행 지원은 향후 확장 항목으로 검토 중입니다.

선택적 로깅 sink는 `src/solar_rs485_monitor/sinks/` 아래의 별도 모듈로 구현하며, Telegram 이벤트 알림은 `src/solar_rs485_monitor/alerts/` 아래에 별도로 구현합니다. 이렇게 해서 인버터 수집 로직과 SQLite, Google Sheets, ThingSpeak, MariaDB, OpenSearch 또는 Elasticsearch 같은 외부 로깅 연동을 분리하고, 알림 채널도 분리해서 관리합니다.

alert 채널은 `src/solar_rs485_monitor/alerts/dispatcher.py`의 registry와 `src/solar_rs485_monitor/alerts/message.py`의 공통 메시지 빌더를 사용합니다. 새 채널(예: Slack)을 추가할 때는 채널 모듈에 `get_config`, `has_config`, `send_alert`를 구현하고 dispatcher에 등록하면 됩니다.
...
`--all-alerts`에서는 `TELEGRAM_BOT_TOKEN`과 대상(`TELEGRAM_CHAT_ID` 또는 `TELEGRAM_CHAT_IDS`)이 하나 이상 설정된 경우에만 Telegram이 활성화됩니다. Telegram 설정 누락을 오류로 확인하고 싶다면 `--telegram`을 명시적으로 사용합니다.
...
기본 동작은 alert 채널에서 정상 측정값을 스킵하고, 장애 이벤트(`fault=1` 또는 `fault_code != 0`)가 감지될 때만 메시지를 전송합니다. 장애 이벤트 메시지에는 주요 측정값과 활성 fault bit 정보가 포함됩니다. 이벤트 시 요약 메시지도 함께 보내려면 `TELEGRAM_SEND_SUMMARY="true"`로 설정하세요.
...
| `fault` | `fault_code`에서 파생 | N/A | 0/1 | 고장 비트(Bit 1~12) 중 하나라도 `1`이면 `1`, 아니면 `0` |
...
- `Telegram request failed`: `TELEGRAM_BOT_TOKEN`, `TELEGRAM_CHAT_ID`, 대상 그룹의 봇 권한, `api.telegram.org`로의 네트워크 접근을 확인합니다.
