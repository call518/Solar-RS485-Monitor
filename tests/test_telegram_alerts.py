...
def test_power_derived_operation_state_sends_standby_transition(monkeypatch) -> None:
    sent_messages = []
    ...
    assert "Solar RS485 Standby Event" in sent_messages[0]
    assert "Fault code: `0`" in sent_messages[0]
    assert "State: `STANDBY (derived)`" in sent_messages[0]
